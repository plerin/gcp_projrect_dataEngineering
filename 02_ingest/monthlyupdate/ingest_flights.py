...
try:
    ...
    import ssl  # Add 43~44 line _ https 접근할 때 필요한 코드(context) -> line 62
...
def download(YEAR, MONTH, destdir):
   ...
   filename = os.path.join(destdir, "{}{}.zip".format(YEAR, MONTH)) # os.path.join은 경로를 만들기 위함, param을 조합해서 하나의 경로로 만든다.
   with open(filename, "wb") as fp:
     response = urlopen(url, PARAMS,context=context)
...
def zip_to_csv(filename, destdir):
   ...
   cwd = os.getcwd() # os.getcwd() = 현재 디렉토리 경로 받아
   os.chdir(destdir) # os.chdir() = 현재 디렉토리 변경
   zip_ref.extractall() # 모든파일 압축해제 : extractall(), 파라미터 없으면 현재 작업 디렉토리에 압축해제
   os.chdir(cwd)
...
def verify_ingest(csvfile):
   ...
   with open(csvfile, 'r') as csvfp:
      ...
      if (firstline != expected_header): # 파일 첫 줄, 필드가 적혀있는 줄이 예상된 header와 다른 경우
         os.remove(csvfile)
      ...
      if next(csvfp, None) == None: # 파일이 비어있을 경우(None) 
         os.remove(csvfile)
